...
const PrincipalFees = () => {
  ...
  // ✅ UPDATED: Real API implementation for fee collection
  const handleCollectFee = async (feeItem) => {
    const confirm = window.confirm(
      `Collect full outstanding amount of ₹${parseFloat(
        feeItem.outstanding_amount
      ).toLocaleString()} for ${feeItem.student?.first_name || "this student"}?`
    );
    if (!confirm) return;

    const studentId = feeItem.student?.id;
    const studentFeeId = feeItem.id; // this is the student_fee_id
    const amount = parseFloat(feeItem.outstanding_amount);
    ...
    const payment_date = today.toISOString().split("T")[0]; // e.g., "2025-01-15"

    // Generate simple transaction ID (you can replace with real one if needed)
    const transaction_id = `TXN${Date.now()}`;

    const payload = {
      student_fee_id: studentFeeId,
      amount: amount,
      payment_mode: "cash", // or prompt user later
      payment_date: payment_date,
      transaction_id: transaction_id,
      remarks: "Collected via Principal Dashboard",
    };

    try {
      const response = await safeFetchJSON(
        `${API_BASE}/api/students/${studentId}/payment`,
        {
          method: "POST",
      ...
      if (response.success) {
        alert(
          `Payment recorded successfully! Receipt: ${
            response.data?.receipt_number || "N/A"
          }`
        );
        // Refresh all data
        await Promise.all([
  ...
  const generateReceiptPDF = (payment) => {
    ...
    y += lineHeight;
    doc.text(
      `Student: ${payment.student_fee?.student?.full_name || "—"}`,
      40,
      y
    );
    y += lineHeight;
    doc.text(
      `Admission No.: ${payment.student_fee?.student?.admission_number || "—"}`,
      40,
      y
    );
    y += lineHeight;
    doc.text(
      `Fee Head: ${payment.student_fee?.fee_structure?.fee_head?.name || "—"}`,
      40,
      y
    );
    y += lineHeight * 2;
      ...
      >
          ...
          <div className="d-flex justify-content-between align-items-center flex-column flex-md-row gap-2">
            ...
            <div className="d-flex gap-2">
              ...
              <Button
                ...
                onClick={() => navigate("/dashboard/principal/fees/collect")}
              >
                + Collect Fee
              </Button>
            </div>
        ...
        <Card.Body className="p-0">
          {loading ? (
            ...
            <Alert variant="danger" className="m-3 rounded-2">
              {error}
            </Alert>
          ) : (
            <>
                    ...
                    <Card key={p.id} className="mb-3 shadow-sm border">
                      <Card.Body>
                        ...
                        <div className="d-flex justify-content-between mb-3">
                          ...
                          <Badge
                            bg={p.status === "success" ? "success" : "warning"}
                          >
                          ...
                          <tr key={p.id}>
                            ...
                            <td>
                              <Badge
                                ...
                                text={p.status === "success" ? "light" : "dark"}
                              >
                                {p.status === "success" ? "PAID" : "PARTIAL"}
                              </Badge>
                            </td>
                    ...
                    <Card key={f.id} className="mb-3 shadow-sm border">
                      <Card.Body>
                        ...
                        <div className="d-flex justify-content-between mb-2">
                          <strong>Paid:</strong>
                          <span>
                            ₹{parseFloat(f.paid_amount).toLocaleString()}
                          </span>
                        </div>
                        <div className="d-flex justify-content-between mb-2">
